...
class OnBoardingViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun checkLoginStatus() {
        ...
        if (isLoggedIn) {
            val existingUser = AuthManager.getUserInfo(context)
            navigateToMain()
        } else {
            _uiState.value = _uiState.value.copy(
                isLoading = false,
    ...
    fun confirmAccountSwitch(onLoginComplete: (Intent) -> Unit) {
        ...
        val accessToken = AuthManager.getAccessToken(context) ?: return
        saveUserAndNavigate(newUser, accessToken, onLoginComplete)
    }

    private fun saveUserAndNavigate(user: com.example.nubo.data.model.UserInfo, token: String, onLoginComplete: (Intent) -> Unit) {
        AuthManager.saveAccessToken(context, token)
        AuthManager.saveUserInfo(context, user)
